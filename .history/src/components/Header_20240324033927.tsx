...
import Menu, { ClickType, MenuItemType } from "./Menu";
...
export type HeaderProps = {
  ...
  handleChangeTab?: (type: ClickType) => void;
  style?: CSSProperties;
  className?: string;
};

const Header: React.FC<HeaderProps> = ({ h = '64px', fixed, filter = 0, borderBottom = true, logo, name, divider = true, navData = [], actived, align, handleChangeTab,style, className }) => {
  return <StyledHeader className={`land-header ${fixed ? 'fixed' : ''} ${className}`} style={style} height={h} filter={filter} borderBottom={borderBottom}>
    ...
    <StyledHeaderNav className="land-header-nav" align={align}>
      <Menu data={navData} actived={actived} handleChangeTab={type =>  handleChangeTab} />
    </StyledHeaderNav>
