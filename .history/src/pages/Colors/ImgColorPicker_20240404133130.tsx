...
const ImgColorPicker: React.FC<Props> = ({}) => {
  ...
  return (
    <div className="flex column items-start gap-12 px-24">
      {/* 上传框 */}
      <Title title="Step 01: 上传图片" type="h3" className="mt-24" /><IconInfoFill/>
      <Title title="点击上传需要提取主体色的图片" type="p" />
      
      <StyleUploadWrap className="flex gap-24 width-100">
        <Uploader
          fileType="image/*"
          onUpload={(url) => {
            setImgUrl(url);
          ...
          desc="点击上传图片或将图片拖拽于此"
          className="flex-1 radius-12 height-100"
        >
          {imgUrl && (
...
const StyleUploadWrap = styled.div`
  min-height: 240px;
`;
