...
const ImgColorPicker: React.FC<Props> = ({}) => {
  ...
  function areColorsSimilar(color1: any, color2: any, threshold = 32) {
    const hsv = tinycolor(`rgb(${color1[0]},${color1[1]},${color1[2]})`).toHsv();
    if (
      (hsv.s < filter/100 && hsv.s < filter/100 && hsv.s < filter/100) ||
      (hsv.v < filter/100 && hsv.v < filter/100 && hsv.v < filter/100)
    ) {
      return true;
    }
