...
const ImgColorPicker: React.FC<Props> = ({}) => {
  ...
  const [uploaderRatio, setUploaderRatio] = useState<any>();
  useEffect(() => {
  ...
  useEffect(() => {
    if (uploaderRef.current) {
      const uploader = uploaderRef.current.getBoundingClientRect();
      setUploaderRatio(uploader.width / uploader.height);
    }
  }, [uploaderRef]);
  // 吸管
  const handlePick = () => {
  ...
  return (
    <div className="flex column items-start gap-32 px-24 pt-32">
      {/* 上传框 */}
      <Flex column gap={24}>
        <Flex column gap={8}>
      ...
      <Flex column gap={24} wrap>
        ...
        <div className="flex flex-wrap width-100">
          ...
          {/* 颜色列表 */}
          <div
            className={`relative flex items-center justify-center flex-wrap gap-12 p-24 width-100`}
          >
...
const StyleColorCardWrap = styled.div<{
  ...
  width: 100%;
  grid-template-columns: ${(props) => props.length < 8 ? 
    `repeat(auto-fit, minmax(${props.length * 40 + 48}px, 1fr))` : 'repeat(auto-fit, minmax(${props.length * 40 + 48}px, 1fr))'};
  &.disabled {
    opacity: var(--opacity-02);
    pointer-events: none;
  }
