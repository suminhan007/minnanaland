...
const ImgColorPicker: React.FC<Props> = ({}) => {
  ...
  function euclideanDistance(color1:any[], color2:any[]) {
    const rDiff = (color1[0] - color2[0]) ** 2;
    const gDiff = (color1[1] - color2[1]) ** 2;
    const bDiff = (color1[2] - color2[2]) ** 2;
    return Math.sqrt(rDiff + gDiff + bDiff);
  }
  function isVisuallySimilar(color1:any[], color2:any[], threshold = 1) {
    const floatColor1 = rgbToFloat(color1);
    const floatColor2 = rgbToFloat(color2);
    const distance = euclideanDistance(floatColor1, floatColor2);
  ...
  const analyzeImageData = (imageData: any) => {
    ...
    for (let i = 4; i < data.length; i += 4) {
      if (
        isVisuallySimilar(`[${data[i]},${data[i+1]},${data[i+2]}]`,`[${data[i-4]},${data[i-3]},${data[i-2]}]`)
        // Number(data[i]) - Number(data[i - 4]) >= 20 ||
        // Number(data[i + 1]) - Number(data[i - 3]) >= 20 ||
        // Number(data[i + 2]) - Number(data[i - 2]) >= 20
      ) {
        let r = data[i];
        let g = data[i + 1];
        let b = data[i + 2];
