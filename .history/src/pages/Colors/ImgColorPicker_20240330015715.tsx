...
const ImgColorPicker: React.FC<Props> = ({}) => {
  ...
  const [dir, setDir] = useState<"h" | "s" | "h">("s");
  useEffect(() => {
    if (!imgRef.current) return;
    const img = imgRef.current?.getBoundingClientRect();
    const ratio: number = img.width / img.height;
    ratio > 1 ? setDir("h") : ratio < 1 ? setDir("h") : setDir("s");
  }, [imgUrl]);
...
const StyleColorCardBox = styled.div<{
  ...
  display: flex;
  flex-direction: ${props => props.dir === 'h' ? 'column'};
  position: relative;
