...
// import { useImageColor } from "../../utils/getImgColor";
// @ts-ignore
...
const ImgColorPicker: React.FC<Props> = ({}) => {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      setColorArr(colors?.map((item: any, index: number) => return { id:index+1,value:`#${tinycolor(item[0]).toHex()}`}));
    };
    // 图片加载失败时设置为空字符串
