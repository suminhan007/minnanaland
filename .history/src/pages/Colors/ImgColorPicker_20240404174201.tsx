...
const ImgColorPicker: React.FC<Props> = ({}) => {
  ...
  const uploaderRatio = useMemo(() => {
    if (!uploaderRef.current) return 1;
  ...
  return (
    <div className="flex column items-start gap-24 px-24 pt-32">
      ...
      <Flex column gap={24}>
        ...
        {/* 取色配置 */}
        {imgUrl && (
          <div className="flex gap-8 relative hover-pop">
            <Ratio
              text="过滤中性色"
              checked={filter !== 0}
              onChange={(val) => {
            ...
            />
            <Pop content="勾选后将自动过滤饱和度或纯度低于 10 的颜色<br/>支持输入自定义过滤范围（≤35）" />
            {/* <Input
              maxLength={2}
              value={String(filter)}
              w={32}
              onChange={(val) => {
                Number(val) <= 35 ? setFilter(Number(val)) : setFilter(filter);
              }}
              onBlur={() => getImgColor(imgUrl)}
            /> */}
            <SliderBlock style={{width:'100px',height:'24px'}} />
          </div>
        )}
        {/* 颜色列表 */}
        {colorArr.length !== 0 ? (
