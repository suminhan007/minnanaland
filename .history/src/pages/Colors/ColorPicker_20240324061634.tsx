...
const ColorPicker: React.FC<Props> = ({}) => {
  ...
  const analyzeImageData = (imageData: any) => {
    ...
    let topRgbValues = sortedRgbCounts.slice(0, 8);
    // 输出或处理结果
    return topRgbValues;
  };
  /* 展示的颜色数据 */
  ...
  return (
    <div className="StyleColorPickerWrap flex column items-center">
      <StyleUploadBtn className="StyleUploadBtn hover-translate-y-4 transition mt-48 cursor-pointer">
        <Uploader fileType="image/*" onUpload={(url) => setImgUrl(url)}>
          <div className="color-gray-3">上传图片</div>
        </Uploader>
      </StyleUploadBtn>
      <img
        src={imgUrl}
