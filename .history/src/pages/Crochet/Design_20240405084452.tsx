...
const Design: React.FC<Props> = ({}) => {
  ...
  const [pixelArr, setPixelArr] = useState<string[]>(Array.from({ length: x * y }));
  return (
    <StyleDesignWrap className="p-24">
      ...
      <div style={{ width: "fit-content" }}>
        <StyleDesignGrid className="grid border radius-12" x={x} y={y}>
          {pixelArr?.map((item) => (
            <StyleDesignGridPixel className="border" style={{backgroundColor:item.value}}></StyleDesignGridPixel>
          ))}
        </StyleDesignGrid>
