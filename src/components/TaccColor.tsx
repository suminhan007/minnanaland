...
import { Dropdown, DropdownContent, DropdownProps, Input } from '@tencent/spaui';
import React, { CSSProperties, ReactNode, useCallback, useEffect, useMemo, useState } from 'react';
import styled from 'styled-components';
import tinycolor from 'tinycolor2';
import TaccButton from './TaccButton';
import Icon from '@tencent/od-icon';
import TaccNumberInput from './TaccNumberInput';
...
const opacityIcon = (
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <path d="M8 12C10.2091 12 12 10.2091 12 8C12 5.79086 10.2091 4 8 4V12Z" fill="var(--od-gray-03)" />
  </svg>
);

const strokeIcon = (
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
    <rect x="1" y="14" width="14" height="1" rx="0.5" fill="var(--od-gray-03)" />
...
const TaccColor: React.FC<Props> = ({
  ...
  const handleHexInputChange = (val: string) => {
    ...
    if (val.length === 6) {
      ...
      setCurrentColor(`rgba(${rgb.r},${rgb.g},${rgb.b},${currentOpacity / 100})`);
    ...
    >
      ...
      <DropdownContent style={{ width: 'fit-content', background: 'transparent' }}>
        <StyleColorContainer className={`relative ${STYLE_CONTENT_ID}`}>
          ...
          {(input || opacity || stroke) && (
            <div className="flex items-center mt-16 gap-8">
              {input && (
                ...
                >
                  <div className='flex items-center justify-start gap-8 overflow-hidden' style={{ minWidth: '52px' }}><StyledColorIcon className='flex items-center justify-center overflow-hidden' />更多</div>
                  <Icon name='right-outlined' color='var(--od-gray-03)' />
                </TaccButton>
              )}
              {stroke && (
                <TaccNumberInput
          ...
          {showMoreColor &&
            <StyledColorPanel
              className={`StyledColorPanel absolute flex flex-column gap-8 bg-white radius-6 p-16 ${showMoreColor ? 'show' : ''}`}
            >
              <StyledColorGrid
                ...
                onMouseDown={(e: any) => { e.preventDefault(); handleThumb(e); setMove(true) }}
                onMouseUp={() => { setMove(false) }}
              >
                <div
                  className='color-thumb absolute'
                  style={{ left: s * 228, top: 84 - v * 84 }}
                ></div >
              </StyledColorGrid>
              <StyledColorSlider
                className='radius-6'
                ...
                onChange={(e: any) => handleHChange(e.target.value)}
              />
              <StyledOpacityWrap className="relative radius-6">
                <StyledColorSlider
                  ...
                  max={100}
                  value={inputOpacity}
                  onChange={(e: any) => { setInputOpacity(e.target.value); onChange?.(getRgbaColor(currentColor, e.target.value / 100)) }}
                  className='radius-6 opacity'
                  currentColor={svColor}
                />
              </StyledOpacityWrap>
              <div className='flex gap-8 mt-8'>
                ...
                {opacity && (
                  <TaccNumberInput
                    ...
                    height={30}
                    value={inputOpacity}
                    onChange={(_e: any, v: string) => { Number(v) <= 100 && setInputOpacity(v) }}
                    onBlur={() => onChange?.(getRgbaColor(currentColor, currentOpacity))}
                    className="flex-1"
                  />
                )}
...
const StyleColorItem = styled.div`
  ...
  &.active {
    outline: 1px solid var(--od-blue-06);
  }
  cursor: pointer;
  &.null {
    position: relative;
